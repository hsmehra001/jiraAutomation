...
def norm(s):
    """Normalize status names for flexible matching."""
    return s.lower().strip().replace(" ", "").replace("-", "").replace(".", "")


# Completion targets (flexible)
COMPLETION_TARGETS = {
    "done",
    "completed",
    "closed",
...
class TaskProcessor:
    ...
    def _process_single_subtask(self, subtask: Issue, current_username: str) -> Dict:
        ...
        try:
            ...
            if norm(current_status) == "inprogress":
                ...
                # Step 3: Move to any completion status
                completed = False
                for target in COMPLETION_TARGETS:
                    # Try each possible completion status
                    if self.jira_client.transition_issue(subtask, target.capitalize()):
                        actions.append(f"Moved to {target.capitalize()}")
                        completed = True
                        break

                if not completed:
                    logger.warning(f"Could not transition {subtask.key} to any completion status")

            # Step 4: Skip if already in a completion status
            elif norm(current_status) in COMPLETION_TARGETS:
                return {
